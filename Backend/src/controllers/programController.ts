...
const mockPrograms = [
  {
    ...
    ngo: { _id: 'mock-ngo-1', name: 'Seva Foundation' },
  ...
  },
  {
    ...
    ngo: { _id: 'mock-ngo-2', name: 'Green Earth NGO' },
  ...
  },
  {
    ...
    ngo: { _id: 'mock-ngo-3', name: 'Health Connect' },
...
export const getPrograms = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (mongoose.connection.readyState !== 1) {
      ...
      res.status(200).json({
        success: true,
        data: mockPrograms.slice(0, limitNum),
        pagination: {
          currentPage: pageNum,
          totalPages: 1,
          totalRecords: mockPrograms.length,
          hasNextPage: false,
          hasPrevPage: false
        },
        message: 'Programs retrieved successfully (mock data - database not available)'
      });
      return;
    }
